...
Encapsulates all GA logic: fitness evaluation, day-aligned crossover,
constraint repair, and the NSGA-II evolutionary loop. Used by both
the training script and the inference service.

GPU-accelerated batch fitness evaluation via PyTorch.
"""
...
class GAOptimizer:
    ...
    Encapsulates fitness evaluation, genetic operators, and the
    evolutionary loop. Instantiate with a SchedulingProblem,
    then call run() with a GAConfig.

    Fitness evaluation is GPU-accelerated: the entire population is
    batch-evaluated as a single 2D tensor using scatter_add_.
    ...
    def _precompute_tensors(self) -> None:
        """Pre-compute constant tensors used in batch fitness evaluation."""
        p = self._problem
        d = self._device
        ...
        # Per-shift hour values: lens[shift_types] → shape (num_shifts,)
        ...
        # Unavailability mask: (num_shifts,) bool tensor
        # True where the assigned shift position has an unavailable day
    ...
    def batch_fitness(
        ...
        Args:
            individuals: List of schedules (each a list of employee indices).

        Returns:
            List of (imbalance, violations, back_to_back) tuples.
        """
        p = self._problem
        d = self._device
        n = len(individuals)

        pop_t = torch.tensor(individuals, dtype=torch.long, device=d)  # (n, num_shifts)

        # Hours per employee: scatter_add shift hours by employee index
        lens = self._shift_hour_values.unsqueeze(0).expand(n, -1)  # (n, num_shifts)
        hours = torch.zeros(n, p.num_employees, dtype=torch.float64, device=d)
        hours.scatter_add_(1, pop_t, lens)

        # Objective 1: Jain's imbalance (vectorized over population)
        ...
        # Objective 2: Constraint violations (vectorized)
        exceed = (hours - self._max_hours_t).clamp(min=0).sum(dim=1)

        # Unavailability violations: check each shift position
    ...
    def fitness(self, individual: list[int]) -> tuple[float, float, float]:
        """Single-individual fitness (delegates to batch_fitness)."""
    ...
    def repair(self, individual: list) -> list:
        """Repair operator: fix assignments where employee is unavailable."""
        p = self._problem
    ...
    def _evaluate_population(
        ...
        """Batch-evaluate all invalid individuals and assign fitness values."""
    ...
    def run(
        ...
        Args:
            ...
            verbose: Print generation statistics every 10 generations.

        Returns:
            GAResult with best schedule, Pareto front, and logbook.
        """
        if config is None:
            config = GAConfig()
        ...
        for gen in range(1, config.generations + 1):
            ...
            # Batch evaluate invalid individuals
        ...
        # Best = lowest sum of objectives from Pareto front
