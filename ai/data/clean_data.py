...
Usage:
    python -m data.clean_data
    python -m data.clean_data --input-dir ai/data --output-dir ai/data/cleaned
"""
...
def parse_time_value(val) -> float | None:
    """Parse a time value from the raw Excel data.

    Handles: float (9.0, 17.0), malformed (1530.0 → 15.5), string ("x"), NaN.
    Returns hour as float or None if off-duty.
    """
    if pd.isna(val):
        return None
    ...
    # Fix Excel auto-format: 1530.0 → 15:30 → 15.5
    if val > 24:
        hours = int(val) // 100
        minutes = int(val) % 100
...
def compute_shift_hours(start: float, end: float) -> float:
    """Compute shift duration in hours, handling overnight shifts."""
    if end > start:
        return end - start
    else:
        # Overnight shift (e.g., 23→7 = 8 hours)
        return (24.0 - start) + end


def classify_shift(start: float) -> int:
    """Classify shift type by start time.
...
def load_and_clean_excel(file_path: str) -> pd.DataFrame:
    ...
    # Find employee columns (pairs of _s and _e)
    all_cols = df.columns.tolist()
    employees = sorted(
        set(col.replace("_s", "").replace("_e", "") for col in all_cols if col.endswith(("_s", "_e")))
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(Path(__file__).parent / "cleaned"),
