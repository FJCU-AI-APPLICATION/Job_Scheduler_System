...
class ModelRegistry:
    ...
    def _load_metadata(self) -> dict:
        """Load model metadata to determine algorithm class."""
    ...
    def _get_algorithm_class(self) -> type[BaseAlgorithm]:
        """Determine which SB3 algorithm class to use for loading."""
    ...
    def load_model(self, checkpoint_name: str) -> BaseAlgorithm:
        """Load an SB3 model from a checkpoint .zip file."""
        if checkpoint_name in self._cache:
            return self._cache[checkpoint_name]

        # Strip .zip extension if provided for path resolution
        stem = checkpoint_name.removesuffix(".zip")
        checkpoint_path = self.model_dir / stem

        # SB3 adds .zip automatically, check both with and without
