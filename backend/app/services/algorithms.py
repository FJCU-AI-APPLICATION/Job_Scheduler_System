...
def process_schedule(data: ScheduleInput) -> ComputedSchedule:
    """
    Process scheduling data to compute a schedule.

    For each day, assigns all available employees to each shift.
    Uses attribute access throughout (no dict["key"] access).
    """
